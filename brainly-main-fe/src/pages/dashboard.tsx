...
//                 {filterContent.map(({ type, link, title , _id}) => (
//                   <motion.div
//                     key={_id}
//                     className="transform transition-all duration-200 hover:scale-105 hover:shadow-xl"
//                     whileHover={{ scale: 1.05 }}
//                   >
//                     <Card type={type} link={link} title={title} id={_id}  refresh = {refresh} />
//                   </motion.div>
//                 ))}
//               </div>
//             </div>
...
export function Dashboard() {
  ...
  // Refresh content after the modal closes (e.g., after creating content)
  useEffect(() => {
    if (!modelOpen && typeof refresh === 'function') {
      refresh();
    }
  }, [modelOpen, refresh]);

  const filterContent =
    type === 'All' ? contents : contents.filter((content: any) => content.type === type);
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 relative overflow-hidden">
      ...
      <div className="flex-1 transition-all duration-300 ease-in-out ml-60 relative z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button
              onClick={async () => {
                ...
                // Keep the same share path; avoid hardcoding host
                const shareUrl = `${window.location.origin}/api/v1/brain/${response.data.hash}`;
                copy(shareUrl);
                toast.success('Link copied to clipboard!');
              }}
              variant="secondary"
        ...
        <div className="max-w-7xl mx-auto px-8 py-8 z-0">
          ...
          >
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-6">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-blue-600 mb-1">{filterContent.length}</div>
                    <div className="text-sm text-gray-600 font-medium">
                      {type === 'All' ? 'Total Items' : `${type} Items`}
                    </div>
                  </div>
                  <div className="h-12 w-px bg-gray-200"></div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-green-600 mb-1">{contents.length}</div>
                    <div className="text-sm text-gray-600 font-medium">All Content</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-lg font-semibold text-gray-900 mb-1">Current Filter</div>
                  <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                    {type}
                  </div>
                </div>
          ...
          {filterContent.length > 0 ? (
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-gray-500">{filterContent.length} items found</div>
              </div>

              {/* Smaller, responsive gap; consistent cell height */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mx-8 gap-6 md:gap-8">
                {filterContent.map(({ type, link, title, _id }: any) => (
                  <motion.div
                    key={_id}
                    className="h-full transform transition-all duration-200 hover:scale-105 hover:shadow-xl"
                    whileHover={{ scale: 1.05 }}
                  >
                    {/* If Card doesn't accept className, this still works; otherwise pass className="h-full" */}
                    <Card type={type} link={link} title={title} id={_id} refresh={refresh} />
                  </motion.div>
                ))}
              </div>
          ...
          <div className="mt-16 pt-8 border-t border-gray-200">
            <div className="text-center text-sm text-gray-500">
              <p className="font-medium">Powered by your creativity • Built with ❤️</p>
            </div>
          </div>
        </div>
      </div>
